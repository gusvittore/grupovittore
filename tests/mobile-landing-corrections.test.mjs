...
test("mobile headings use the shared mobile sizing pattern without desktop changes", () => {
  assert.match(mobileCss, /--mobile-section-title:\s*clamp\(30px,\s*8vw,\s*38px\);/);
  assert.match(mobileCss, /--mobile-long-title:\s*clamp\(28px,\s*7\.2vw,\s*36px\);/);
...
test("hero and form titles have mobile-specific manual line groupings", () => {
  ...
  assert.match(page, /className="section-title section-title-left form-title-mobile"[\s\S]*Agende uma reuni[\s\S]*o online,[\s\S]*receba um diagn[\s\S]*stico comercial[\s\S]*e descubra onde a sua empresa[\s\S]*pode estar[\s\S]*perdendo vendas[\s\S]*dentro do pr[\s\S]*prio processo\./);
