...
function Sidebar({ onCategory }: { onCategory: (category: BlogCategory) => void }) {
  const recommended = BLOG_POSTS.filter((post) => post.recommended);
    ...
    <aside className="space-y-5 lg:sticky lg:top-28 lg:self-start">
      ...
      <section className="rounded-[18px] border border-[#b29157]/35 bg-[#fffdf9] p-6 sm:p-7">
        ...
        <div className="mt-4 divide-y divide-[#b29157]/20">
          {BLOG_CATEGORIES.filter((category): category is BlogCategory => category !== "Todos").map((category) => (
            <button
              key={category}
              type="button"
      ...
      <section className="rounded-[18px] border border-[#b29157]/35 bg-[#fffdf9] p-6 sm:p-7">
        ...
        <div className="mt-4 divide-y divide-[#b29157]/20">
          {recommended.map((post) => (
            <a key={post.slug} href="#" onClick={(event) => event.preventDefault()} className="group block py-4 first:pt-0 last:pb-0">
...
const categoryBlocks: CategoryBlock[] = [
  {
    category: "Vendas",
    description: "Processo comercial, acompanhamento de oportunidades, diagnóstico de gargalos e conversão em receita.",
    articleDescription: "Antes de aumentar o investimento em mídia, entenda onde as oportunidades se perdem no processo comercial.",
    postSlug: "gargalos-comerciais-antes-de-investir-em-trafego",
  },
  {
    category: "Marketing",
    description: "Posicionamento, campanhas, aquisição e comunicação para atrair oportunidades mais qualificadas.",
    articleDescription: "Marketing, vendas e tecnologia precisam atuar de forma conectada para transformar atenção em oportunidades reais.",
    postSlug: "marketing-vendas-tecnologia-conectar-tres-areas",
  },
  {
    category: "Materiais Gráficos",
    description: "Presença física, branding, materiais impressos e comunicação profissional fora do digital.",
    articleDescription: "Materiais impressos fortalecem a presença física da marca e ajudam a transmitir profissionalismo em cada contato.",
    postSlug: "materiais-graficos-fortalecem-presenca-da-marca",
  },
  {
    category: "Tecnologia",
    description: "CRM, automações, inteligência artificial e organização de dados para operações mais eficientes.",
    articleDescription: "Um CRM bem utilizado organiza oportunidades, acompanhamentos e dados para dar mais controle à operação comercial.",
    postSlug: "crm-controle-da-operacao-comercial",
  },
];
...
export function BlogHome() {
  ...
  const highlightedPost = filteredPosts.find((post) => post.featured) ?? filteredPosts[0];
  ...
  function selectCategory(category: (typeof BLOG_CATEGORIES)[number]) {
    ...
    <main className="min-h-screen overflow-x-clip bg-[#fbf8f4] text-[#071026]">
      ...
      <section id="artigos" className="mx-auto w-full max-w-[1500px] px-5 pb-16 sm:px-8 sm:pb-24 lg:px-12 xl:px-16">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-16">
          ...
          <Sidebar onCategory={selectCategory} />
        </div>
      </section>
      ...
      <section id="categorias-editoriais" className="border-t border-[#b29157]/20 bg-[#fffdf9]">
        <div className="mx-auto w-full max-w-[1500px] px-5 py-16 sm:px-8 sm:py-20 lg:px-12 xl:px-16">
          ...
          <div className="mt-9 space-y-5">
            {categoryBlocks.map(({ category, description, articleDescription, postSlug }) => {
              const post = BLOG_POSTS.find((item) => item.slug === postSlug);
              if (!post) return null;
